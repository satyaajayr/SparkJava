...
public class ProblemStatement1 {
	...
	public static void main(String[] args) {
		...
		// configure spark
		SparkConf conf = new SparkConf().setAppName("ProblemStatement1").setMaster("local[*]");
		...
		JavaRDD<String> filteredLines = lines.filter(x -> !x.split(",")[0].equals("VendorID") && !x.isEmpty())
				.filter(x -> {
					String[] cols = x.split(",");
					return cols[0].equals("2") && cols[1].equals("2017-10-01 00:15:30")
							&& cols[2].equals("2017-10-01 00:25:11") && cols[3].equals("1") && cols[4].equals("2.17")
									? true
									: false;
				});

		// Saving the output in a Text file.
		filteredLines.saveAsTextFile("OutputPS1");
 ...
 * 
 * public String VendorID; public String tpep_pickup_datetime; public String
 * tpep_dropoff_datetime; public String passenger_count; public String
 * trip_distance; public String RatecodeID; public String store_and_fwd_flag;
 * public String PULocationID; public String DOLocationID; public String
 * payment_type; public String fare_amount; public String extra; public String
 * mta_tax; public String tip_amount; public String tolls_amount; public String
 * improvement_surcharge; public String total_amount;
 * 
 * }
 * 
 * @SuppressWarnings("serial") public static class ParseCsv implements
 * FlatMapFunction<Iterator<String>, YelloTaxiTrip> { public
 * Iterator<YelloTaxiTrip> call(Iterator<String> lines) throws Exception {
 * ArrayList<YelloTaxiTrip> tripData = new ArrayList<YelloTaxiTrip>();
 * 
 * while (lines.hasNext()) { String line = lines.next(); try { String[] cols =
 * line.split(",");
 * 
 * YelloTaxiTrip ytp = new YelloTaxiTrip();
 * 
 * if (!cols[0].equals("VendorID") && !line.isEmpty()) { ytp.VendorID = cols[0];
 * ytp.tpep_pickup_datetime = cols[1]; ytp.tpep_dropoff_datetime = cols[2];
 * ytp.passenger_count = cols[3]; ytp.trip_distance = cols[4]; ytp.RatecodeID =
 * cols[5]; ytp.store_and_fwd_flag = cols[6]; ytp.PULocationID = cols[7];
 * ytp.DOLocationID = cols[8]; ytp.payment_type = cols[9]; ytp.fare_amount =
 * cols[10]; ytp.mta_tax = cols[11]; ytp.tip_amount = cols[12]; ytp.tolls_amount
 * = cols[13]; ytp.improvement_surcharge = cols[14]; ytp.total_amount =
 * cols[15];
 * 
 * tripData.add(ytp); } } catch (Exception e) { System.out.
 * println(" Received Exception while parsing the csvFile. Exception is: " +
 * e.toString()); e.printStackTrace(); } } return tripData.iterator(); } }
 * 
 * @SuppressWarnings("serial") public static class YtToString implements
 * FlatMapFunction<Iterator<YelloTaxiTrip>, String> { public Iterator<String>
 * call(Iterator<YelloTaxiTrip> ytps) throws Exception { ArrayList<String>
 * tripData = new ArrayList<String>();
 * 
 * while (ytps.hasNext()) { YelloTaxiTrip ytp = ytps.next(); try { StringBuilder
 * s = new StringBuilder(); if (ytp.equals(null)) { s = s.append(""); } else { s
 * = s.append(ytp.VendorID).append(",").append(ytp.tpep_pickup_datetime).append(
 * ",")
 * .append(ytp.tpep_dropoff_datetime).append(",").append(ytp.passenger_count).
 * append(",")
 ...
 * .append(ytp.fare_amount).append(",").append(ytp.extra).append(",").append(ytp
 * .mta_tax)
 * .append(",").append(ytp.tip_amount).append(",").append(ytp.tolls_amount).
 * append(",")
 * .append(ytp.improvement_surcharge).append(",").append(ytp.total_amount); }
 * tripData.add(s.toString()); } catch (Exception e) { System.out.
 * println(" Received Exception while parsing the csvFile. Exception is: " +
 * e.toString()); e.printStackTrace(); } } return tripData.iterator(); } }
 * 
 * @SuppressWarnings("serial") public static class Filter1 implements
 * Function<YelloTaxiTrip, Boolean> { public Boolean call(YelloTaxiTrip ytp) {
 * 
 * return ytp.VendorID.equals("2") &&
 * ytp.tpep_pickup_datetime.equals("2017-10-01 00:15:30") &&
 * ytp.tpep_dropoff_datetime.equals("2017-10-01 00:25:11") &&
 * ytp.passenger_count.equals("1") && ytp.trip_distance.equals("2.17") ? true :
 * false; } }
 */
/*
 * public static String yeptoString(YelloTaxiTrip yep) { StringBuilder s = new
 * StringBuilder(); if (yep.equals(null)) { s = s.append(""); } else { s =
 * s.append(yep.VendorID).append(",").append(yep.tpep_pickup_datetime).append(
 * ",")
 * .append(yep.tpep_dropoff_datetime).append(",").append(yep.passenger_count).
 * append(",")
 ...
 * ",")
 * .append(yep.DOLocationID).append(",").append(yep.payment_type).append(",").
 * append(yep.fare_amount)
 * .append(",").append(yep.extra).append(",").append(yep.mta_tax).append(",").
 * append(yep.tip_amount)
 * .append(",").append(yep.tolls_amount).append(",").append(yep.
 * improvement_surcharge).append(",") .append(yep.total_amount);
 * 
 * } return s.toString(); }
 */
// JavaRDD<YelloTaxiTrip> result = lines.mapPartitions(new
// ParseCsv()).filter(new Filter1());

// JavaRDD<String> stringResult = result.mapPartitions(new YtToString());

// stringResult.saveAsTextFile("PS1Output");
